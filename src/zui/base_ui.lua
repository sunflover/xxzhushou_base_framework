...
function GetUI()
	...
	pageBaseSet:addLabel({text="教练模式    ",size=30})
	pageBaseSet:addRadioGroup({id="radioCoachMode",list="联赛,天梯",select=1,w=35,h=12})
	
	pageBaseSet:nextLine()
	pageBaseSet:addLabel({text="开场换人    ",size=30})
	pageBaseSet:addRadioGroup({id="radioSubstitute",list="开启,关闭",select=1,w=35,h=12})
	pageBaseSet:addLabel({text="（需设置换人规则）",size=20})
	...
	pageBaseSet:nextLine()
	pageBaseSet:nextLine()
	pageBaseSet:addLabel({text="循环场数      ",size=26})
