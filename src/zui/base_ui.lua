...
function GetUI()
	...
	pageBaseSet:nextLine()
	pageBaseSet:nextLine()
	pageBaseSet:addLabel({text="开场换人条件    ",size=30})
	pageBaseSet:addComboBox({id="comboBoxSubstituteConditon",list="主力状态为红,替补状态更好,替补状态好两档",select=1,w=38,h=16, size = 24})
	pageBaseSet:nextLine()
	pageBaseSet:nextLine()
	...
	pageSubstituteSet:addLabel({text="替补 1  -->",size=24})
	pageSubstituteSet:addComboBox({id="comboBoxBench1",list=feildPositionStr,select=0,w=35,h=12, size = 15})
	pageSubstituteSet:addLabel({text="                      位置说明",size=24})
	pageSubstituteSet:nextLine()
	pageSubstituteSet:addLabel({text="替补 2  -->",size=24})
	pageSubstituteSet:addComboBox({id="comboBoxBench2",list=feildPositionStr,select=1,w=35,h=12, size = 15})
	pageSubstituteSet:addLabel({text="            对位换人，替补席从上到下",size=20})
	pageSubstituteSet:nextLine()
	pageSubstituteSet:addLabel({text="替补 3  -->",size=24})
	pageSubstituteSet:addComboBox({id="comboBoxBench3",list=feildPositionStr,select=2,w=35,h=12, size = 15})
	pageSubstituteSet:addLabel({text="        依次为替补1-7号，场上球员严",size=20})
	pageSubstituteSet:nextLine()
	pageSubstituteSet:addLabel({text="替补 4  -->",size=24})
	pageSubstituteSet:addComboBox({id="comboBoxBench4",list=feildPositionStr,select=3,w=35,h=12, size = 15})
	pageSubstituteSet:addLabel({text="        格按照从上到下（请参考编号",size=20})
	pageSubstituteSet:nextLine()
	pageSubstituteSet:addLabel({text="替补 5  -->",size=24})
	pageSubstituteSet:addComboBox({id="comboBoxBench5",list=feildPositionStr,select=4,w=35,h=12, size = 15})
	...
	pageSubstitutePic:addImage({src="substitute.png",w=60,h=100,xpos=0,align="cnter"})
	
	local pageTestting = Page:new(myui,{text = "相关说明",size = 24})
	pageTestting:nextLine()
	pageTestting:addLabel({text="        注意事项：使用请先切换至教练模式页面 ",size=20, align="cnter"})
	pageTestting:nextLine()
	pageTestting:addLabel({text="        已实现的功能：自动循环刷教练天梯模式，支持加时赛和点球，支持开场 ",size=20, align="cnter"})
	pageTestting:nextLine()
	pageTestting:addLabel({text="根据球员状态自动换人调整。 ",size=20, align="cnter"})
	pageTestting:nextLine()
	pageTestting:addLabel({text="        目前为测试期，功能陆续添加中。关于分辨率的问题，目前只适配了16:9 ",size=20, align="cnter"})
	pageTestting:nextLine()
	pageTestting:addLabel({text="的比例，在功能完善后会增加18:9等比例的适配，请其他比例分辨率的用户暂 ",size=20, align="cnter"})
	pageTestting:nextLine()
	pageTestting:addLabel({text="时使用模拟器尝试。 ",size=20, align="cnter"})
	pageTestting:nextLine()
