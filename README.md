...
标签（空格分隔）： 叉叉助手 框架 引擎v1.9

---

## 写在前面
接触叉叉起因是想刷一下实况足球手游（以下简称实况）的小号，这当然没有现成的，刚好看到叉叉提供了开发平台，自己动手就顺理成章了。写完之后发现还蛮有意思，顺便可以搞一下当时很火的SIM天梯的自动操作，于是开始整理代码，形成一个简单的框架，以简便开发和后续复用。
本意是想开发一个尽可能简洁并尽可能通用的框架，但在没有开发过其他游戏脚本的情况下，直接以实况为蓝本进行开发难免会导致很多地方以适应这个游戏为主，且易有疏漏，望多指正。
